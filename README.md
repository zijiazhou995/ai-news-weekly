...
如果希望 AIbase 新闻稳定拿到每条精确详情页链接，先安装一次浏览器抓取依赖：

```bash
npm install --no-save playwright
```

装好后，脚本会用浏览器渲染 AIbase 列表并读取 `/zh/news/数字ID` 详情页链接；如果没有安装，会提示安装命令。
...
4. 阅读网页内容时，可以勾选“符合”，也可以点“修改”写自己的润色版本。网页会先保存在当前浏览器里；点击“导出反馈”后，把导出的 JSON 放进：

```text
feedback/
```

下一次生成周报时，脚本会参考这些反馈：相似内容更容易被选中，同源内容会优先使用你的润色版本。

5. 如果需要看 Markdown 草稿，打开输出文件：

```text
outputs/年份-W周-ai-news-draft.md
```

6. 先看“更符合要求”，再核验“带来源和评分”里的链接。

7. 需要更正式的表述时，把精选短讯复制给大模型，并使用：
